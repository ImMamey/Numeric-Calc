function [X,niter] = newtonRaphson2(x,tol)
...
# se puede pedir numero maximo de funciones
...
for i = 1:100
 ...
 niter=i; X=[X' E]; # niter es el numero de iteraciones y x devuelve la matriz transpuesta de X y el error.
 endfunction

 function [y, jac] = f(x) #linea 14
  y = zeros (3, 1); # tengo un sisema de dos filas con una columnas d epuros zeros
  y(1) = x(1)^2 + x(2)-37; # es la primera funcion
  y(2) = x(1)^2-x(2)^2-5; # es la segunda funcion
  y(3)=x(1)+x(2)+x(3)-3; # es la 3era funcion
